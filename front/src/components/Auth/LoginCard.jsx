...
export default function LoginCard() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.status === "complete") window.location.href = "/mapa";
    } catch (err) {
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 w-full max-w-xs sm:max-w-sm">
        ...
        {step === "email" && (
          <form onSubmit={handleContinueEmail} className="flex flex-col gap-4">
            <p className="text-center text-gray-500 text-sm">Por favor loguearse para continuar</p>
            ...
            <div className="flex items-center gap-2 text-gray-400 text-sm">
              <hr className="flex-1 border-gray-300" /> O <hr className="flex-1 border-gray-300" />
            </div>
            ...
            <button type="button" className="text-gray-600 text-sm text-center hover:underline">
        ...
        {step === "password" && (
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-sm text-gray-600 px-1">contraseña</label>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder="Ingrese contraseña"
        ...
        {step === "register" && (
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            <p className="text-center text-gray-500 text-sm">Creá tu cuenta para continuar</p>
        ...
        {step === "verify" && (
          <form onSubmit={handleVerify} className="flex flex-col gap-4">
            <p className="text-center text-gray-500 text-sm">
              Te enviamos un código a tu email. Ingresalo para confirmar tu cuenta.
            </p>
            <input type="text" placeholder="Código de verificación" value={code}
              onChange={(e) => setCode(e.target.value)}
              className="border border-gray-400 rounded-full py-2 px-4 text-sm outline-none focus:border-blue-500 bg-white/70 text-center tracking-widest" />
            {error && <p className="text-red-500 text-xs text-center">{error}</p>}
            <button type="submit" disabled={loading}
              className="bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-full py-2 transition-colors disabled:opacity-60">
