...
export default function BarrasCategoria({ reportes, titulo = "Reportes por categoría", delay = "300ms" }) {
  ...
  const data = categorias.map(cat => ({
    ...
    total: reportes.filter(r => r.categoria === cat.nombre).length,
  }));
    ...
    >
      ...
      <div className="flex flex-col gap-3">
        {data.map((item) => {
          ...
          const color = CATEGORIA_COLORS[item.name] || "#378ADD";
          return (
            <div key={item.name} className="flex items-center gap-3">
              <img src={item.icon} className="w-5 h-5 shrink-0" alt={item.label} />
